...
#include <packagekit-glib2/pk-package.c>
...
static gboolean
pk_backend_resolve_package (PkBackendJob *self, const gchar *package,
			    GError **error)
{
	alpm_pkg_t *pkg;
...
static gboolean
pk_backend_resolve_thread (PkBackendJob *self)
{
	gchar **packages;
	GError *error = NULL;
...
void
pk_backend_resolve (PkBackendJob *self, PkBitfield filters, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
...
static gboolean
pk_backend_get_details_thread (PkBackendJob *self)
{
	gchar **packages;
	GError *error = NULL;
...
void
pk_backend_get_details (PkBackendJob *self, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
...
static gboolean
pk_backend_get_files_thread (PkBackendJob *self)
{
	gchar **packages;
	GError *error = NULL;
...
void
pk_backend_get_files (PkBackendJob *self, gchar **package_ids)
{
	g_return_if_fail (self != NULL);
	g_return_if_fail (package_ids != NULL);
